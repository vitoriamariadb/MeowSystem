#!/usr/bin/env bash
# assets/temas-de-apps/spotify/manifesto.sh — Catppuccin dentro do Spotify, VIA SPICETIFY.
#
...
meow_app_conferir() {
  ...
  if [ -n "$vb" ] && [ -n "$vi" ] && [ "$vb" != "$vi" ]; then
    ...
    meow_aviso "o backup do spicetify é da versão $vb e o Spotify é a $vi — veja assets/temas-de-apps/spotify/RECUPERACAO.md"
  fi
...
meow_app_aplicar() {
  ...
  if [ -n "$vb" ] && [ -n "$vi" ] && [ "$vb" != "$vi" ]; then
    ...
    meow_info "não conserto sozinho: leia assets/temas-de-apps/spotify/RECUPERACAO.md e rode na mão"
    return "$MEOW_SEM_DEPENDENCIA"
  fi
...
meow_app_reverter() {
  ...
  if [ -n "$vb" ] && [ -n "$vi" ] && [ "$vb" != "$vi" ]; then
    if _spot_backup_fresco; then
      ...
      meow_info "leia assets/temas-de-apps/spotify/RECUPERACAO.md; a volta limpa é 'flatpak install --user --reinstall flathub com.spotify.Client'"
      return "$MEOW_ERRO"
    fi
  fi
